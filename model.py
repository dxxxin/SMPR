...
class SmileEmbedding(nn.Module):
    def __init__(self, input_dim=300, out_dim=64, dropout=0.4):
        super(SmileEmbedding, self).__init__()
        self.ecpf_linear = nn.Sequential(
    ...
    def forward(self, ECPFs):
        R = self.ecpf_linear(ECPFs)
        #decode = self.decode(R)
        return R
...
class Model(nn.Module):
    ...
    def __init__(self, etypes, ntypes, in_feats, hidden_feats, num_heads, dropout):
        ...
        self.smile_embed = SmileEmbedding()

        self.drugs = torch.rand(999, in_feats)
        self.relation = torch.rand(999, 454)

    def forward(self, g=None, x=None, ECPFs=None):
        R = self.smile_embed(ECPFs)
        #self.drugs = self.drugs[:len(ECPFs)]
        #self.relation = self.relation[:len(ECPFs)]

        if g==None or x==None:
            print('load existing D.pkl')
            return R
