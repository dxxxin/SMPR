...
def get_metrics(real_score, predict_score):
    ...
    t = np.array(thresholds.T)[max_index][0]
    #print('thresholds[0][max_index]',t)
    return auc[0, 0], aupr[0, 0], accuracy, f1_score, precision, recall, specificity, t
...
def plot_result_auc(path, label, predict, auc):
    ...
    #plt.show()
    plt.savefig(os.path.join(path, 'result_auc.png'))
    plt.clf()


def plot_result_aupr(path, label, predict, aupr):
    """Plot the ROC curve for predictions.
    Parameters
    ----------
    args: argumentation
    ...
    plt.legend(loc='lower right')
    plt.savefig(os.path.join(path, 'result_aupr.png'))
    #plt.show()
    plt.clf()
